...
Working with the MCL Cluster 
```{r}
ISIPMCL <- dplyr::filter(MCL, grepl ('^clust_1814$|clust_9740', cluster))
ISIPMCL <- ISIPMCL [-c(1, 3:27, 46:75)]
...
names(ISIPMCL) <- c("name","NoFe_0.5", "Fe_0.5", "NoFe_3", "Fe_3", "NoFe_6", "Fe_6" )

ISIPMCL <-  melt (data = ISIPMCL,id.vars = "name", measure.vars = c(2:7))
ISIPMCL <- separate(ISIPMCL, col = variable, into = c("Fe", "temperature"))

ggplot(ISIPMCL, aes(x=temperature, y=value, colour = name, shape = Fe ))+
geom_point(size = 3)+
 geom_line(size = 1, aes (x=temperature, y=value, group = interaction(Fe, name)))+
...
#guides(colour = name)+
scale_shape_manual(name = "Treatment", labels = c("+ Fe","- Fe"), values = c("circle", "triangle")) +
  scale_color_manual (name = "MCL-Cluster", labels = c("ISIP1", "ISIP-2A"), values = c("red", "black"))
#geom_jitter(width = 0.11, size =2)+
#labs (color = "Treatment") +
 #scale (labels = c("djdj", "jdjdss"))+
#labs (shape = "Transcript")
#scale_y_log10()
