...
Doing some T1 stuff
```{r}
#all_ORF_t1 <- all_ORF[,c(47, 48, 91, 178, 96,98,99,101,102,104)]

#all_MCL_t1 <- all_MCL[,c(48, 47, 148,55:59,61,62,64,65,67)]
#all_MCL_t1 <- all_MCL[,c(48, 47, 148,55,57:59,61,62,64,65,67)] #no T0A


all_MCL_t1 <- all_MCL[,c(48, 47, 148,55,59,61,62,64,65,67)] #no T0


names(all_MCL_t1) <- c("cluster","taxa", "clustername","ice-edge", "T1_noFe_0","T1_Fe_0","T1_noFe_3","T1_Fe_3","T1_noFe_6","T1_Fe_6")

all_MCL_t1_frag <- dplyr::filter (all_MCL_t1, grepl('Fragila', taxa))

#then let's do a heatmap for some photosynthetic stuff 

#light harvesintg
light_harvesting_t1 <- dplyr::filter (all_MCL_t1_frag, grepl('light harvesting|LHC|Lhc|lhc|harvesting|clust_332 |clust_1236 |clust_80 |clust_402 |clust_1044 |clust_3282 |clust_1194 |clust_3219 |clust_1084 |clust_646 |clust_1938', clustername))

#Plastocy
#light_harvesting_t1 <- dplyr::filter (all_MCL_t1_frag, grepl('clust_1820 ', clustername))

#light_harvesting_t1 <- dplyr::filter (all_MCL_t1_frag, grepl('ISIP|isip|Isip|clust_9740 |clust_343 |clust_1814 |clust_1154 ', clustername))

#light_harvesting_t1 <- dplyr::filter (all_MCL_t1_frag, grepl('cold|Cold', clustername))

#light_harvesting_t1 <- dplyr::filter (all_ORF_t1_frag, grepl('ammonium', name))

light_harvesting_t1 <- light_harvesting_t1[c(3:10)]
light_harvesting_t1 <- data.frame(light_harvesting_t1[,-1], row.names = light_harvesting_t1 [,1])

light_harvesting_t1 <- as.matrix ((light_harvesting_t1))

#then you're ready to plot your heatmap!
  heatmap.2 (light_harvesting_t1, 
             trace = "none",
             density = "none", 
             col = bluered(100),
             cexRow = 0.5, 
...
#png(filename="Fe_Ven.png",width=38*0.75, height=30*0.75, units="cm", res = 900)
...
Domoic Acid
```{r}
DABC <- dplyr::filter (all_ORF_sub, grepl('contig_254441_113_1024_+|contig_1174518_67_906_+|contig_732674_1_798_-|contig_633551_118_1041_+|contig_627399_1_787_-|contig_1862_1_732_-|contig_660976_1_1043_+|contig_253357_26_973_+|contig_827413_1_313_-|clust_2216 ' ,name))
...
DABD <- dplyr::filter(pseudo_frag_ORF, grepl('contig_596040_24_938_-|contig_625510_955_1881_-|contig_625510_3536_4051_+|contig_82244_85_1023_+|contig_596040_947_1648_-|contig_82244_1395_1877_+|contig_680152_23_630_-|contig_1109498_1_752_-|contig_1147495_1_1248_+|contig_625510_1971_2528_-|contig_795490_14_403_-|contig_602505_1_924_+|contig_644544_26_931_+|contig_1338988_954_1898_-|contig_631357_332_904_-|contig_895090_1_705_-|contig_584212_1095_1886_+|contig_708803_1_573_-|contig_1174000_279_929_-|clust_12542 |clust_7510 |clust_2795 |P450 |clust_2216 ' ,name))

volcanoplot_interactive(DABD,"temp_6C_vs_0C_foldchange", "temp_6C_vs_0C_pvalue", "temp_6C_vs_0C_DABD")

volcanoplot_interactive(DABD,"Fe_vs_noFe_foldchange", "Fe_vs_noFe_pvalue", " Fe_vs_NoFe_DABD")
...
frag_GO <- dplyr::filter (all_ORF_GO, grepl('Pseudo', Taxa.group))
...
goFrame=GOFrame(goframeData_frag,organism="Pseudo-nitzschia")
goAllFrame=GOAllFrame(goFrame)
...
##creating a KO dataframe. The GoStats package requires two columns for the 
...
#Then the keggframe is passed through the KEGG collection to standardize things."we pass this kind of custom information around, we
#have chosen to support geneSetCollection objects from GSEABase package." 
...
#utliers <- dplyr::filter(frag_all_KO, grepl("-1", temp_6C_vs_0C_de))

outliers<- frag_all_KO[which(frag_all_KO$temp_6C_vs_0C_de <0 ) ,]
...
#we then select only the pathways with >9 unique KO ID's associated with them
hyperG_KO_filtered <- hyperG_KO[which(hyperG_KO$Size >4),]
...
#enrichedORFs_df<- data.frame((sapply(enrichedORFs, '[', seq(max(sapply(enrichedORFs, length))))))


allORFs <- geneIdUniverse(Over) # this gets list of all ORFS used in analysis
allORFs_df<- data.frame((sapply(allORFs, '[', seq(max(sapply(allORFs, length))))))

allORFs_df$placeholder <- "placeholder"

####````` Segway````## delete after
testest <- allORFs_df[538]
testest <- na.exclude(testest)
names(testest) <- "orf_id"

testtest_2 <- merge (hyperG_KO_filtered, frag_all_KO, by ="KO")
length(unique(testtest_2$KO_pathway))
#testtest_3 <- dplyr::filter(frag_KO, grepl("Ribosome", KO_pathway))

#``````````````#

allORFs_df <- melt(allORFs_df, id.vars="placeholder")
allORFs_df<- na.exclude(allORFs_df)
names(allORFs_df)<- c("placeholder", "KO", "orf_id")
allORFs_df$KO <- str_sub(allORFs_df$KO, start=2)
allORFs_df$KO <- paste0("K", allORFs_df$KO)

length(unique(allORFs_df$KO))

test <- merge(allORFs_df, hyperG_KO_filtered, by = c("KO"))
length(unique(test$KO))
test_2 <- merge(test, frag_all_KO, by = c("KO", "orf_id"))
length(unique(test_2$KO))
test_3 <- merge(test_2, kegg_2, by = c ("KO"))

length(unique(test_3$KO))


#then choose only the pathways with >9 unique KOid associated with them

# pathways<-unique(test$KO_pathway)
# 
# test_filtered<-data.frame()
# for (i in 1:length(pathways)){
#   temp<-base::subset(test,KO_pathway==pathways[i])
#   if (length(unique(temp$KO))>1) {test_filtered<-rbind(test_filtered,temp)}
# }
# 
# length(unique(test_filtered$KO_pathway))


# y = str_wrap(KO_pathway, 80) 
ggplot(test_3, aes(x = temp_6C_vs_0C_foldchange, y = str_wrap(KO, 70), color = str_wrap(temp_6C_vs_0C_de, 20)))+
  geom_jitter(alpha=0.4, size = 2,  height = 0.2)+
  scale_color_manual(values = c("red", "grey", "red"))+
  geom_vline (xintercept = c(0), col = "black", lty = 2, lwd=0.3 )+
