...
scatterplot3d(heatshock3dmelt[5:3],
                highlight.3d =TRUE, 
              main = "3D scatterplot", 
              xlab = "Iron",
              ylab =  "Temperature",
...
plot3d (heatshock3dmelt$Iron, 
        heatshock3dmelt$Temperature,
        heatshock3dmelt$Value,
        xlab ="Iron",
        ylab = "Temperature", 
        zlab = "value",
...
fixdata(ISIP1, 'ISIP1', "ISIP1", "ISIP1mean", 0.02, 0.09)
...
ISIP1 <- dplyr::filter (ORF2, grepl('ISIP1', name))
fixdata(ISIP1, 'ISIP1', "ISIP1", "ISIP1mean")
...
##Volcano  
```{r}
volcano <- read.csv("MCLclustervolcano.csv", header = TRUE, sep = ",")

#removing rows with no values
volcanoclean <- volcano[-which(volcano$ann_desc == ""),]

#make all descriptions unique
volcanoclean$ann_desc <- sub('[.]', '_', make.names(volcanoclean$ann_desc, unique = TRUE))

with(ORF, plot(ORF$X16_vs_17.log2fc, -log10(ORF$X16_vs_17.fdr), pch=20))+
abline (h=1.0, col = "blue", lty = 2, lwd=1)+
  abline (v= c(-1,1), col = "blue", lty = 2, lwd=1)

P = ggplot (data = ORF, aes(x= X12_vs_13.log2fc, y=-log10(X12_vs_13.fdr))) +
  geom_point (alpha=0.4, size = 1.75)

P+geom_text_repel(data =  filter (ORF, ORF$X12_vs_13.fdr>2 ), aes (label = ORF$orf_id))
